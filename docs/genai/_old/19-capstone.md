---
id: capstone
title: "Capstone: Research Copilot"
...
**Research Copilot** — a knowledge assistant over a personal or team library of PDFs, web pages, CSVs and YouTube transcripts. It:
...
Every chapter appears somewhere.
...
flowchart TB
    subgraph ING["Ingestion · offline"]
        ...
        TS --> EM["Embeddings"] --> VS[("Chroma<br/>persisted")]
    end
    subgraph QRY["Query time"]
        ...
        RT -->|live info| AG["ReAct agent<br/>search · calculator · weather"]
...
**Goal.** A single function that takes any source and returns chunks, with metadata rich enough to cite.
...
Changing the embedding model invalidates the whole index. Pin it, and rebuild deliberately if you ever change it.
:::
...
**Chapters:** [prompts](/docs/genai/prompts), [chains](/docs/genai/chains), [runnables](/docs/genai/runnables), [RAG](/docs/genai/rag).
...
Add, in order, measuring after each:
...
def build_hybrid_retriever(store, chunks):
    semantic = store.as_retriever(search_type="mmr",
...
**Done when:** each tool works standalone, and each has a docstring that tells a model exactly when to use it.

:::danger That `eval`
The character whitelist blocks the obvious attacks and is **not** a sandbox. For anything real, use a proper expression parser. Never hand a model unrestricted `eval`.
:::
...
**Chapters:** [tool calling](/docs/genai/tool-calling), [agents](/docs/genai/agents).
...
**Done when:** a question needing library lookup *and* arithmetic resolves in one call, with a visible trace.
...
Then branch with `RunnableBranch` on `destination`.

**Done when:** "hello" never triggers a retrieval, and "what's the exchange rate today" never hits the document store.

Routing is where cost control starts. Most questions do not need your most expensive path.

## Milestone 9 — Evaluation

**Goal.** Numbers instead of impressions.

**Chapter:** [advanced concepts](/docs/genai/advanced-concepts).
...
def run_eval(chain, retriever, golden):
    ...
    return evaluate(Dataset.from_list(rows),
...
- **LangGraph rewrite** — rebuild the agent as a state graph with human approval before web search.
...
- **MCP server** — expose the library as an MCP server so any client can query it.
